...
MemMgrAllocatorMapEntry *_memory_manager_allocmap_insert(ConstStr id, const ByteSize capacity, const MemMgrAllocType type) {
    ...
    // insert new entry
    {
        ...
        map_->_entries[map_->_count]._type = type;
...
VoidPtr memory_manager_allocate(MemoryManager *manager, const ByteSize size, const MemoryTag tag) {
    if (!state || !state->_initialized)
        return NULL;
...
VoidPtr memory_manager_deallocate(MemoryManager *manager, const ByteSize size, const MemoryTag tag) {
    if (!state || !state->_initialized)
        return NULL;
    ...
    switch (entry_->_type) {
    case ALLOCTYPE_ARENA:
        // arena allocators don't do individual deallocations
        return NULL;
...
VT_API VoidPtr memory_manager_construct_allocator(MemoryManager *manager, ConstStr id, const ByteSize capacity,
    ...
    MemMgrAllocatorMapEntry *constructed_entry_ = _memory_manager_allocmap_insert(id, capacity, type);
...
VT_API Bool memory_manager_destruct_allocator(MemoryManager *manager, ConstStr id) {
    return _memory_manager_allocmap_remove(id);
}
...
Bool memory_manager_report(const MemoryManager *manager, ConstStr id) {
    if (!state || !state->_initialized) {
...
void memory_manager_report_all(const MemoryManager *manager) {
    if (!state || !state->_initialized) {
        misc_console_setforeground_rgb(255, 0, 0);
        misc_console_writeln("Memory Manager is not initialized.");
        misc_console_reset();
