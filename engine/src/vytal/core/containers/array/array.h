...
VT_API Array __vtimpl_ctnr_darr_construct_custom(const ByteSize data_size, const VoidPtr allocator, const MemMgrAllocType alloc_type, const ByteSize capacity);
...
VT_API VT_INLINE Bool container_array_isfull(Array array) { return (container_array_length(array) == container_array_capacity(array)); }

#define container_array_construct(type) __vtimpl_ctnr_darr_construct(sizeof(type))
#define container_array_construct_custom(type, allocator, alloc_type, capacity) __vtimpl_ctnr_darr_construct_custom(sizeof(type), allocator, alloc_type, capacity)
